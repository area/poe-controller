...
public class ControllerOutputSettings implements Loggable {
	...
	private void init() {
		...
		try {
			...
			for (Mapping mapping : config.getMapping()) {
				...
				log(2, mappingKey.getComponentName());
				log(2, mapping);
			}
	...
	public List<Mapping> getRelevantMappings(MappingKey mappingKey, Event event) {
		...
		if (isDigitalButton == true && isPressed == false) {
			...
			log(2, getMappingKeysByNameAndId(mappingKey));
			return getMappingKeysByNameAndId(mappingKey).stream()
					...
					.filter(mapping -> mapping.hasAction() && (mapping.getAction().hasKey() || mapping.getAction().hasMouseAction() ))
					.collect(Collectors.toList());
		}
